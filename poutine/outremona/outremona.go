...
func main() {
	...
	sig := <-c
	log.Println("Got signal: ", sig, ", exiting Outremona service.")
	wp.Wait()
	s.Close()
}

type OutremonaServer struct {
	serv http.Server
}

// Setup Http Server
func NewOutremonaServer() OutremonaServer {
	sm := mux.NewRouter()
	...
	getP.HandleFunc("/outremona", Process)

	s := http.Server{
		Addr:         ":" + os.Getenv("OUTEREMONA_PORT"), // configure the bind address
...
// Start server
func (s *OutremonaServer) Start() {
	go func() {
		log.Println("Starting Outremona server on port " + os.Getenv("OUTEREMONA_PORT"))
...
// Index - Check Status of Outremona service
func Index(w http.ResponseWriter, r *http.Request) {
	w.Header().Add("Content-Type", "application/json")
	ToJSON(Status{Message: "Outremona service working."}, w)
	return

}

// Process - adds request to the cheese queue
func Process(w http.ResponseWriter, r *http.Request) {
	...
	wp.Do(func() error {
		...
		time.Sleep(1 * time.Second) // simulate squeezeing cheese
