...
func main() {
	...
	sig := <-c
	log.Println("Got signal: ", sig, ", exiting Oldoporto service.")
	wp.Wait()
	s.Close()
}

type OldoportoServer struct {
	serv http.Server
}

// Setup Http Server
func NewOldoportoServer() OldoportoServer {
	sm := mux.NewRouter()
	...
	getP.HandleFunc("/oldoporto", Process)

	s := http.Server{
		Addr:         ":" + os.Getenv("OLDOPORTO_PORT"), // configure the bind address
...
// Start server
func (s *OldoportoServer) Start() {
	go func() {
		log.Println("Starting Oldoporto server on port " + os.Getenv("OLDOPORTO_PORT"))
...
// Index - Check Status of Oldoporto service
func Index(w http.ResponseWriter, r *http.Request) {
	w.Header().Add("Content-Type", "application/json")
	ToJSON(Status{Message: "Oldoporto service working."}, w)
	return

}

// Process - adds request to the gravy queue
func Process(w http.ResponseWriter, r *http.Request) {
	...
	wp.Do(func() error {
		...
		time.Sleep(5 * time.Second) // simulate stirring gravy task
